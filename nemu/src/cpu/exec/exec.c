...
make_group(gp2,
  ...
  /* 0xf6, 0xf7 */
make_group(gp3,
    IDEX(test_I, test), EMPTY, EX(not), EX(neg), EX(mul), EX(imul1), EX(div), EX(idiv))
...
make_group(gp4,
  ...
  /* 0xff */
make_group(gp5,
    EX(inc), EX(dec), EX(call_rm), EX(call_rm), EX(jmp_rm), EX(jmp_rm), EX(push), EMPTY)
...
opcode_entry opcode_table [512] = {
  ...
  /* 0x24 */	EMPTY, IDEX(I2a, add), EMPTY, EMPTY,
  ...
  /* 0x98 */	EX(cltd), EX(cltd), EMPTY, EMPTY,
  ...
  /* 0xe8 */	IDEX(J, call), IDEX(J, jmp), IDEX(E, jmp_rm), IDEXW(J, jmp, 1),
  ...
  /* 0x9c */	EMPTY, EMPTY, EMPTY, IDEX(J, jcc),
